...
const argsname = require('fn-args');
const debug = require('debug')('hc-koa');
class Application extends BaseApp {
  ...
  init(frameworkConfig, frameworkOptions, appConfig, envConfig) {
    debug('application start init, envConfig', envConfig);
  ...
  useErrorMiddleware() {
    const customErrorHandler = this.config.errorHandler;
    const wrapJson = this.options.wrapResponseJson;
    const defaultHttpErrorCode = this.options.defaultHttpErrorCode;
    this.koa.router.use(async (ctx, next) => {
      // plug json()
      ctx.json = function (body) {
        if (body) {
          // TODO: koa body 支持buffer，stream和字符串
          ctx.body = {
            code: 'SUCCESS',
            data: body,
            rid: ctx.rid
      ...
      try {
        ...
        res.statusCode = (httpCode >= 100 && httpCode < 600 ? httpCode : 500);
        // res.setHeader('content-type', mime);
        this.log.error('REQUEST_ERROR method:', req.method, 'url:', req.url, 'message:', err.stack || err);
  ...
  run(cb, config) {
    ...
    if (this.config.fallbackHandler) {
      this.koa.use(this.config.fallbackHandler);
    }
  ...
  */
    ...
    middlewareList.forEach(mid => {
      ...
      generatedMiddlewares.forEach((middleware) => {
        try {
          if (midConfig.router) {
            this.koa.use(mount(
              this.config.prefix + midConfig.router,
              middleware
            ));
          } else {
  ...
  initMiddleware(midConfig, middlewares) {
    ...
    switch (type) {
      ...
      case 'array': {
        ...
        /* eslint-disable */
      async function combineMiddleware(ctx, next) {
        for (let i = 0; i < subMids.length; i++) {
          const instance = subMids[i];
          if (instance.match(ctx)) {
            return instance.call(null, ctx, next);
          }
        }
      ...
      /* eslint-enable */
        ...
        combineMiddleware.match = function (ctx) {
          for (let i = 0; i < subMids.length; i++) {
            if (subMids[i].match(ctx)) {
              return true;
            }
          }
          return false;
    ...
    return mds.map((middleware) => {
      ...
      if (middleware.constructor.name !== 'GeneratorFunction') {
        ...
        switch (middleware.length) {
          ...
          case 2: {
            let aNames = argsname(middleware);
            // not a fn(ctx,next) type
            if (!['ctx', 'context'].includes(aNames[0].toLowerCase())) {
  ...
  mountRouter() {
    ...
    this.koa.use(this.koa.router.routes());
    this.koa.use(this.koa.router.allowedMethods());
  }

  getRouter() {
    return this.koa.router;
  }
